...
def send_order(self, ticker, qty, side="BUY"):
    try:
        ...
        # ✅ 가격 0원 방어: 매수/매도 모두 가격 수신 실패 시 주문 차단
        if curr_p <= 0:
...
“”“yfinance 간헐적 빈 DataFrame 대응: 최대 3회 재시도”””
...
```
    ...
    if spy_ohlc.empty: raise ValueError("SPY 데이터 수신 실패 (3회 재시도 초과)")
    if vix_data.empty: raise ValueError("VIX 데이터 수신 실패 (3회 재시도 초과)")
...
if current_hour in [20,0]:
    ...
    if action in ["ENTER", "ROTATE"] and top2:
        ...
        # ✅ 실제 매도 성공한 종목만 state에서 제거 (고스트 포지션 방지)
        remaining_holdings = []
        for h in rot_state.get('holdings', []):
            q = trader.get_holdings(h['ticker'])
            if q > 0:
                sell_res = trader.send_order(h['ticker'], q, "SELL")
                if sell_res.get('rt_cd') == '0':
                    cp = trader.get_current_price(h['ticker']); ret = (cp - h.get('entry_price', cp)) / max(h.get('entry_price', cp), 1) * 100
                    pd.DataFrame([{"Date": now_kst.strftime("%Y-%m-%d %H:%M"), "Action": "SELL", "Ticker": h['ticker'], "Qty": q, "Price": cp, "RetPct": round(ret, 2)}]).to_csv(ROTATION_HISTORY_FILE, mode='a', header=not os.path.exists(ROTATION_HISTORY_FILE), index=False)
                else:
                    print(f"❌ ROT EXIT 매도 실패: {h['ticker']} - {sell_res.get('msg1')}")
                    remaining_holdings.append(h)  # 실패 종목은 state 유지
        rot_state.update({"in_market": len(remaining_holdings) > 0, "holdings": remaining_holdings})
        save_rotation_state(rot_state)
        msgs.append("🚨 ROT 하락장 청산")
    msgs.append(f"🧠 AI: {ask_gemini(u_sig, r_sig)}"); await tg_send(token_v, chat_id, "\n".join(msgs))

elif current_hour in [1,2]:
    # ✅ period='5d'로 받고 오늘 날짜 데이터만 필터링 (period='1d'는 당일 장중 보장 안 됨)
    ...
    if not spy_int.empty:
        ...
        # 오늘 날짜(KST 기준 전날 = 미국 당일) 데이터만 추출
        today_us = (now_kst - timedelta(hours=13)).date()  # KST 01시 = 미국 전날 낮
        if spy_int.index.tz is not None:
            spy_today = spy_int[spy_int.index.tz_convert(None).date == today_us]  # type: ignore
        else:
            spy_today = spy_int[spy_int.index.date == today_us]  # type: ignore
        # 오늘 데이터가 충분하면 당일 기준, 없으면 전체 마지막 데이터로 fallback
        spy_check = spy_today if len(spy_today) >= 5 else spy_int
        day_ret = (float(spy_check['Close'].iloc[-1]) / float(spy_check['Open'].iloc[0])) - 1
        print(f"🔍 긴급탈출 체크 - SPY 당일 등락: {day_ret*100:.2f}%")
        if day_ret <= -0.03:
            q_u = trader.get_holdings(TRADE_TICKER)
            if q_u > 0:
                sell_res = trader.send_order(TRADE_TICKER, q_u, "SELL")
                if sell_res.get('rt_cd') == '0':
                    ...
                    # ✅ 긴급탈출 후 STATE_FILE 저장 (기존엔 누락되어 있었음)
                    with open(STATE_FILE, 'w') as f:
                        json.dump({"in_market": False, "last_exit_price": exit_p}, f)
                    print(f"✅ UPRO 긴급매도 완료: {q_u}주 @ ${exit_p:.2f}")
                else:
                    print(f"❌ UPRO 긴급매도 실패: {sell_res.get('msg1')}")
            if rot_state.get('in_market'):
                # ✅ 실제 매도 성공한 종목만 holdings에서 제거 (고스트 포지션 방지)
                remaining_holdings = []
                for h in rot_state.get('holdings', []):
                    q_h = trader.get_holdings(h['ticker'])
                    if q_h > 0:
                        sell_h = trader.send_order(h['ticker'], q_h, "SELL")
                        if sell_h.get('rt_cd') == '0':
                            ...
                            print(f"✅ ROT 긴급매도 완료: {h['ticker']} {q_h}주")
                        else:
                            print(f"❌ ROT 긴급매도 실패: {h['ticker']} - {sell_h.get('msg1')}")
                            remaining_holdings.append(h)  # 실패한 종목은 state에 유지
                    # q_h == 0 이면 이미 없는 종목 → state에서도 제거
                rot_state.update({"in_market": len(remaining_holdings) > 0, "holdings": remaining_holdings})
                save_rotation_state(rot_state)
            await tg_send(token_v, chat_id, "🚨 긴급 탈출 실행 완료")
    else:
        print("⚠️ 긴급탈출: SPY 5분봉 데이터 수신 실패, 탈출 스킵")
