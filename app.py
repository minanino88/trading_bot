...
def ask_gemini(u_sig, r_sig):
    ...
    try:
        ...
        p = f"UPRO {u_sig}, ROT {r_sig['action']}, TOP2 {r_sig.get('top2')}. Korean 150 chars: 1.시장평가 2.리스크"
...
async def run_trading():
    ...
    if current_hour == 20 or os.getenv('GITHUB_ACTIONS') == 'true':
        ...
        msgs = [f"🤖 <b>통합봇 [{now_kst.strftime('%m/%d %H:%M')} KST]</b>", f"잔고: ${bal:,.2f}"]
        ...
        if u_sig in ["KEEP", "RE-ENTER"] and qty_upro == 0 and cur_p_upro > 0:
            ...
            if buy_qty >= 1:
                ...
                else: msgs.append(f"❌ UPRO 매수실패: {str(res)[:150]}")
            ...
            else: msgs.append(f"❌ UPRO 매도실패: {str(res)[:150]}")
        ...
        if action in ["ENTER", "ROTATE"]:
            if rot_state['in_market']:
                for h in rot_state['holdings']:
                    ...
                    if qty_h > 0:
                        ...
                        ret_pct = (cur_p_h - h.get('entry_price', cur_p_h)) / max(h.get('entry_price', cur_p_h), 1) * 100
                        msgs.append(f"✅ ROT 매도 {h['ticker']}: {qty_h}주 ({ret_pct:+.1f}%)")
                        pd.DataFrame([{"Date": now_kst.strftime("%Y-%m-%d %H:%M"), "Action": "SELL", "Ticker": h['ticker'], "Qty": qty_h, "Price": cur_p_h, "RetPct": round(ret_pct, 2)}]).to_csv(ROTATION_HISTORY_FILE, mode='a', header=not os.path.exists(ROTATION_HISTORY_FILE), index=False)
                time.sleep(2)
            new_holdings, per_stock = [], (rot_budget * 0.95) / len(top2) if top2 else 0
            for ticker in top2:
                cur_p_t = trader.get_current_price(ticker)
                buy_qty = int(per_stock / cur_p_t) if cur_p_t > 0 else 0
                if buy_qty >= 1:
                    res = trader.send_order(ticker, buy_qty, cur_p_t, "BUY")
                    if res.get('rt_cd') == '0':
                        msgs.append(f"✅ ROT 매수 {ticker}: {buy_qty}주 @ ${cur_p_t:.2f}")
                        new_holdings.append({"ticker": ticker, "qty": buy_qty, "entry_price": cur_p_t})
                        pd.DataFrame([{"Date": now_kst.strftime("%Y-%m-%d %H:%M"), "Action": "BUY", "Ticker": ticker, "Qty": buy_qty, "Price": cur_p_t, "RetPct": 0}]).to_csv(ROTATION_HISTORY_FILE, mode='a', header=not os.path.exists(ROTATION_HISTORY_FILE), index=False)
            rot_state.update({"in_market": True, "holdings": new_holdings, "entry_date": now_kst.strftime("%Y-%m-%d")})
            ...
            for h in rot_state['holdings']:
                ...
                if qty_h > 0:
                    res = trader.send_order(h['ticker'], qty_h, cur_p_h, "SELL")
                    ret_pct = (cur_p_h - h.get('entry_price', cur_p_h)) / max(h.get('entry_price', cur_p_h), 1) * 100
                    msgs.append(f"✅ ROT 청산 {h['ticker']}: {qty_h}주 ({ret_pct:+.1f}%)")
                    pd.DataFrame([{"Date": now_kst.strftime("%Y-%m-%d %H:%M"), "Action": "SELL", "Ticker": h['ticker'], "Qty": qty_h, "Price": cur_p_h, "RetPct": round(ret_pct, 2)}]).to_csv(ROTATION_HISTORY_FILE, mode='a', header=not os.path.exists(ROTATION_HISTORY_FILE), index=False)
            rot_state.update({"in_market": False, "holdings": []})
            save_rotation_state(rot_state)
        else: msgs.append(f"ROT: {action} | TOP2: {', '.join(top2)}")
        ...
        if not spy_int.empty:
            ...
            if day_ret <= -0.03:
                qty_upro, cur_p_upro = trader.get_holdings(TRADE_TICKER), trader.get_current_price(TRADE_TICKER)
                if qty_upro > 0:
                    res = trader.send_order(TRADE_TICKER, qty_upro, cur_p_upro, "SELL")
                ...
                if rot_state['in_market']:
                    for h in rot_state['holdings']:
                        cur_p_h, qty_h = trader.get_current_price(h['ticker']), trader.get_holdings(h['ticker'])
