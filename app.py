...
# 원본과 동일: price 인자 받아서 지정가 주문 (APBK1507 에러 방지)
...
{“in_market”: True, “last_exit_price”: 0}
...
“best_trade”: round(max(trades), 2), “worst_trade”: round(min(trades), 2),
...
# Bot 텔레그램 / 지정가 주문 / 01시 긴급매도 / 로테이션 주문
        ...
        if day_ret <= -0.03:
            ...
            if rot_state['in_market']:
                for h in rot_state['holdings']:
                    ...
                    qty_h   = trader.get_holdings(h['ticker'])
                    if qty_h > 0:
                        trader.send_order(h['ticker'], qty_h, cur_p_h, "SELL")
                rot_state['in_market'] = False
                rot_state['holdings']  = []
                save_rotation_state(rot_state)
                if bot:
...
with tab1:
    ...
    c2.metric("ROT Action",    r_signal['action'],
              f"TOP2: {', '.join(r_signal['top2'])}" if r_signal['top2'] else "-")
    ...
    fig.add_trace(go.Candlestick(
        ...
        low=spy126['Low'], close=spy126['Close'], name="SPY"), row=1, col=1)
    ...
    if r_signal.get('scores'):
        ...
        sc_df = pd.DataFrame([
            {"Ticker": t, "Score": s, "TOP2": "⭐" if t in r_signal['top2'] else ""}
...
# ── Tab2 ──
with tab2:
    for label, perf in [("UPRO 트렌드 (70%)", upro_perf),
                         ("모멘텀 로테이션 (30%)", rot_perf)]:
        st.subheader(label)
        ...
        if perf['equity_curve']:
            ...
            fig_eq = go.Figure(go.Scatter(
                ...
                fill='tozeroy', line=dict(color='#3fb950')))
            fig_eq.update_layout(template='plotly_dark', height=250,
                                 margin=dict(t=10, b=10), yaxis_title="Equity")
...
with tab3:
    ...
    if not df_upro.empty:
        st.dataframe(df_upro.tail(20), use_container_width=True, hide_index=True)
    else:
        st.info("거래 기록 없음")
    st.subheader("로테이션 거래 로그")
    if not df_rot.empty:
        st.dataframe(df_rot.tail(20), use_container_width=True, hide_index=True)
    else:
        st.info("거래 기록 없음")
```
